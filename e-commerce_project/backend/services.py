...
async def bootstrap_admins(db: AsyncSession):
    ...
    Parses comma-separated lists from settings.
    """
    # 1. Parse lists from Settings
    usernames = [u.strip() for u in settings.ADMIN_USERNAMES.split(",") if u.strip()]
    passwords = [p.strip() for p in settings.ADMIN_PASSWORDS.split(",") if p.strip()]

    if len(usernames) != len(passwords):
        logger.warning(
            f"Admin Bootstrap Mismatch: Found {len(usernames)} usernames and {len(passwords)} passwords. "
            "Seeding will stop at the shortest list length."
        )
...
async def admin_login_service(db: AsyncSession, username: str, password: str):
    ...
    # Uses 'admin' as default role in token, verified by RBAC in core.py
    return create_access_token(subject=admin.username, role=admin.role.value if hasattr(admin.role, 'value') else "admin")
...
async def create_nowpayments_invoice(order: Order, user_email: str) -> str:
    ...
    payload = {
        ...
        # Uses os.getenv directly for URLs not managed in Pydantic settings yet, or could migrate
        "ipn_callback_url": f"{os.getenv('BACKEND_URL', 'http://localhost:8000')}/api/webhooks/nowpayments",
        "success_url": f"{os.getenv('FRONTEND_URL', 'http://localhost:3000')}/checkout/success",
        "cancel_url": f"{os.getenv('FRONTEND_URL', 'http://localhost:3000')}/checkout/fail"
    }
...
async def handle_nowpayments_webhook(db: AsyncSession, payload: dict, signature: str):
    ...
    # 1. Verify Signature (Implementation simplified)
    if not signature: 
