...
# UPDATE: Import settings to use dynamic path resolution
...
# We use settings.FRONTEND_DIR derived dynamically in core.py
# This prevents "File Not Found" errors on Render/Docker.
...
def load_html_files():
    """
    Pre-loads HTML files into memory on startup using dynamic paths.
    Drastically reduces latency for high-traffic frontend routes.
    """
    logger.info(f"Pre-loading frontend assets from: {settings.FRONTEND_DIR}")
    
    pages = ["index.html", "product.html", "cart.html", "profile.html", "checkout.html", "login.html", "register.html"]
    
    for page in pages:
        # CRITICAL FIX: Use settings.FRONTEND_DIR to resolve path correctly
...
app = FastAPI(
    ...
    version="2.1.0", # Bumped for Dynamic Pathing
    docs_url="/docs",
    redoc_url="/redoc",
    lifespan=lifespan,
    openapi_tags=[
...
# UPDATE: Use settings.STATIC_DIR to ensure correct path on Render
if os.path.exists(settings.STATIC_DIR):
    app.mount("/static", StaticFiles(directory=settings.STATIC_DIR), name="static")
else:
    logger.warning(f"Static directory not found at {settings.STATIC_DIR}. Assets will fail.")
