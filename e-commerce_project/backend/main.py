...
from services import (
    ...
    handle_btcpay_webhook_service, # Renamed or consolidated in services
...
app = FastAPI(
    ...
    version="1.2.0", # Bumped for Multi-Product & Digital Delivery
    docs_url="/docs",
    redoc_url="/redoc",
    lifespan=lifespan,
    openapi_tags=[
...
# B. Security: CORS
# Strictly validate origins in production
origins = []
if os.getenv("ADMIN_FRONTEND_URL"):
    origins.append(os.getenv("ADMIN_FRONTEND_URL"))
...
@webhook_router.post("/btcpay")
async def btcpay_webhook_route(request: Request, db: AsyncSession = Depends(get_db)):
    """Handle BTCPay Server notifications."""
    sig = request.headers.get("BTCPay-Sig")
    payload = await request.json()
    await handle_btcpay_webhook_service(db, payload, sig)
    return {"status": "ok"}
