...
# --- DYNAMIC PATH RESOLUTION (Self-Contained) ---
# Calculates absolute paths relative to this file to avoid "File Not Found"
# errors on Render/Docker/Production.
# Assumed Structure:
#   /root
#     /backend/main.py
#     /frontend/index.html

# 1. Get the directory containing main.py (.../backend)
BASE_DIR = os.path.dirname(os.path.abspath(__file__))

# 2. Get the project root (.../root)
PROJECT_ROOT = os.path.dirname(BASE_DIR)

# 3. Resolve the sibling 'frontend' directory
FRONTEND_DIR = os.path.join(PROJECT_ROOT, "frontend")

# 4. Resolve Upload Temp Dir (Local to backend)
UPLOAD_DIR = os.path.join(BASE_DIR, "temp_uploads")

# Log the resolved paths for debugging in production logs
logger.info(f"Path Resolution -> Base: {BASE_DIR}")
logger.info(f"Path Resolution -> Frontend: {FRONTEND_DIR}")
...
def load_html_files():
    """
    Pre-loads HTML files into memory on startup using the locally resolved absolute path.
...
app = FastAPI(
    ...
    version="2.1.1", # Bumped for Dynamic Path Resolution
    docs_url="/docs",
    redoc_url="/redoc",
    lifespan=lifespan,
    openapi_tags=[
...
async def register_page():
    return HTML_CACHE.get("register.html", "<h1>Not Found</h1>")
...
# UPDATE: Use the locally resolved FRONTEND_DIR to ensure correct path on Render.
