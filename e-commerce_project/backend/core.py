...
# - Dynamic Path Resolution for Static Assets (Fixes Render/Docker path errors)
# - Cryptography: Argon2 (No length limit, GPU-resistant)
...
# Structure assumed:
# /project_root
#   /backend (contains core.py)
#   /frontend (contains assets)

_CORE_FILE_PATH = os.path.abspath(__file__)          # /project/backend/core.py
_BACKEND_DIR = os.path.dirname(_CORE_FILE_PATH)      # /project/backend
_PROJECT_ROOT = os.path.dirname(_BACKEND_DIR)        # /project
...
class Settings(BaseSettings):
    ...
    # --- Asset Paths (Dynamic Defaults) ---
    # These default to the sibling 'frontend' folder but can be overridden by .env
    FRONTEND_DIR: str = Field(default=_RESOLVED_FRONTEND_PATH)
    STATIC_DIR: str = Field(default=_RESOLVED_FRONTEND_PATH)
...
try:
    ...
    # Validation logging
    if not os.path.exists(settings.FRONTEND_DIR):
        logging.warning(f"Frontend directory not found at resolved path: {settings.FRONTEND_DIR}")
    else:
        logging.info(f"Frontend directory resolved to: {settings.FRONTEND_DIR}")
